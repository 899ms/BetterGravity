...
async function forkFromSnapshot(agentService, request) {
  ...
  try {
    ...
    if (/not a valid branch name|check-ref-format|failed to create worktree/i.test(wtMsg)) {
      plugin.ui.toast({
        title: "Worktree branch unavailable",
        body: "Git branches cannot contain spaces. Forking into current workspace instead...",
        kind: "info",
        duration: 4000
      });
  ...
  const run = async () => {
    try {
      ...
      try {
        ...
        if (target === 2 && /not a valid branch name|check-ref-format|failed to create worktree/i.test(errMsg)) {
          plugin.ui.toast({
            title: "Worktree branch unavailable",
            body: "Git branches cannot contain spaces. Forking into current workspace instead...",
            kind: "info",
